...
def _normalise_signature_value(value):
    return None if pd.isna(value) else value


def _terminal_signature(row: pd.Series) -> tuple:
    return tuple(
        _normalise_signature_value(row.get(column))
        for column in _TERMINAL_SIGNATURE_COLUMNS
    )
...
def merge_terminal_states(
    ...
    A finished result replaces a live, scheduled, or uncertain state. A newer
    official terminal record may correct an older terminal result, while an older
    terminal record never overwrites a newer one.
    """
