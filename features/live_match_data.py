    ...
    *reports: TerminalReconciliationReport,
    ...
    return TerminalReconciliationReport(
        restored_match_ids=tuple(
            sorted({match_id for report in reports for match_id in report.restored_match_ids})
        ),
        corrected_match_ids=tuple(
            sorted({match_id for report in reports for match_id in report.corrected_match_ids})
        ),
        appended_match_ids=tuple(
            sorted({match_id for report in reports for match_id in report.appended_match_ids})
        ),
    )
