...
def load_qualification_data() -> dict:
    ...
    return {
        ...
        "group_tables": load_csv(data_dir / "current_group_tables.csv"),
...
def render_qualification_page(data: dict, refresh_label: str) -> None:
    ...
    render_official_data_caption(data["prices"])
...
def render_idle_qualification_lab() -> None:
    ...
    if should_auto_refresh(data["matches"]):
        st.rerun()
    render_qualification_page(data, "No match is active; CupMarket checks again every five minutes.")
