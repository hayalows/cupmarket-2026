...
def patch_page_copy() -> None:
    replacements = {
        "features/tournament_pulse_page.py": [
            ("<div class=\"cm-eyebrow\">CupMarket 2026 · Tournament pulse</div>", "<div class=\"cm-eyebrow\">CupMarket 2026</div>"),
            ("<h1>See what happened. Open what matters.</h1>", "<h1>Tournament overview</h1>"),
            (
        ...
        "features/match_hub_v2.py": [
            ("<div class=\"cm-eyebrow\">CupMarket 2026 · Fixtures & results</div>", "<div class=\"cm-eyebrow\">CupMarket 2026</div>"),
            ("<h1>Every match, one clear path.</h1>", "<h1>Matches and results</h1>"),
            (
        ...
        "pages/1_Match_Intelligence.py": [
            ("<h1>Open the match. Understand the consequences.</h1>", "<h1>Match intelligence</h1>"),
        ],
        "pages/2_Qualification_Lab.py": [
            ("<h1>What does the next result change?</h1>", "<h1>Qualification scenarios</h1>"),
        ],
    }
    for path, items in replacements.items():
        text = read(path)
        for old, new in items:
            text = replace_once(text, old, new, f"copy in {path}")
        write(path, text)
...
def patch_analysis() -> None:
    ...
    text = read(path)
    start = text.index('st.set_page_config(page_title="CupMarket Analysis Lab"')
    prefix = text[:start]
    body = """st.set_page_config(page_title="CupMarket Analysis", page_icon="💡", layout="wide")
inject_styles(DATA_DIR.parent)
...
def cleanup_old_release_files() -> None:
    for relative in (
        "scripts/apply_ux_ia_refresh.py",
        "scripts/repair_ux_patch_payload.py",
        ".github/workflows/apply-ux-ia-refresh.yml",
    ):
...
def main() -> None:
    ...
    patch_tests()
    patch_readme()
    cleanup_old_release_files()
    print("Direct CupMarket UX and IA branch patch applied.")
